...
// getByID behandelt GET /ausleihe/{id}
func getByID(svc *Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		ausleihe, err := svc.GetByID(id)
		if err != nil {
			shared.WriteError(w, err)
			return
		}
		shared.WriteJSON(w, http.StatusOK, ausleihe)
	}
}

// getByMitgliedID behandelt GET /ausleihe/mitglied/{mitgliedID}
func getByMitgliedID(svc *Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		ausleihen, err := svc.GetByMitgliedID(mitgliedID)
		if err != nil {
			shared.WriteError(w, err)
			return
		}
		shared.WriteJSON(w, http.StatusOK, ausleihen)
	}
}

// create behandelt POST /ausleihe
func create(svc *Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var input AusleiheInput
		if err := json.NewDecoder(r.Body).Decode(&input); err != nil {
			shared.WriteJSON(w, http.StatusBadRequest, map[string]string{"message": "Ungueltige JSON-Daten"})
			return
		}
		ausleihe, err := svc.Create(&input)
		if err != nil {
			shared.WriteError(w, err)
			return
		}
		shared.WriteJSON(w, http.StatusCreated, ausleihe)
	}
}

// update behandelt PUT /ausleihe/{id}
func update(svc *Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		ausleihe, err := svc.Update(id, &input)
		if err != nil {
			shared.WriteError(w, err)
			return
		}
...
// delete behandelt DELETE /ausleihe/{id}
